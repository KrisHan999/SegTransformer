...
class Dataset3d(Dataset):
    ...
    def _load_mask(self, pid, img_shape):
        ...
        if self.with_issue_air_mask:
            for mask_type in ['issue_mask', 'air_mask']:
                if os.path.isfile(os.path.join(self.data_dir, '%s_%s.nrrd' % (pid, mask_type))):
                    m, _ = nrrd.read(os.path.join(self.data_dir, '%s_%s.nrrd' % (pid, mask_type)))
                    mask_array.append(m)
                    mask_flag.append(True)
                else:
                    mask_array.append(np.zeros(img_shape))
